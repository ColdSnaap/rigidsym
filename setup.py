...
case = uniq_general_comb(sym_no, ratio, rigid_type, rigid_max)
case_origin = copy.deepcopy(case)
# make combine case list [single1, single2]
case_dir = copy.deepcopy(case)
for keys in case_dir:
    exist_single2 = 'single2' in case_dir[keys]
    if exist_single2 == False:
        case_dir[keys]['single'] = case_dir[keys].pop('single1')
    else:
        single_list = case_dir[keys]['single1'] + case_dir[keys]['single2']
        case_dir[keys]['single'] = single_list
        del case_dir[keys]['single1']
        del case_dir[keys]['single2']
        ...
        while dis_transform_return != 'yes':
            ...
            for i in WyckoffSingle:
                ...
                with open(f'single_{n_single}', 'w') as single_write:
                    if exist_single2 == False:
                        single_write.write(compound_single_atom[0])
                    
                    elif exist_single2 == True and n_single <= len(case_origin[key]['single1']):
                        single_write.write(compound_single_atom[0])
                    elif exist_single2 == True and n_single > len(case_origin[key]['single1']):
                        single_write.write(compound_single_atom[1])
